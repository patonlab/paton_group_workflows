...
try:  # Ensure compatibility between openbabel 2.4 and openbabel 3.1 
    import pybel
...
class TurbomoleInput(object):
    ...
    def check_atom_list(self):
        ...
        if atoms and '*' not in atoms:
            # '*' not in atoms is a patch for a bug in openbabel-3.0.0
            # this bug is not present in 2.4.1 nor 3.1.1
            return atoms
    ...
    def _check_auxbasis_availability_per_atom(self):
        ...
        self.use_jkbasis_as_jbasis = False
        found_list  = []
        default_auxbasis = self.auxbasis.get('default','NotDefined')
        for atom in self.atoms:
            sym = atom.lower()
            auxbasis = self.auxbasis.get(sym,default_auxbasis)
    ...
    def write_atom_attr_menu(self):
        ...
        default_b = self.basis.get('default','')
        for atom in self.atoms: 
            basis = self.basis.get(atom,default_b)
            output += f'b "{atom}" {basis}\n'        # Add basis set per atom
        if self.ecp is not None: 
            ecps = [(k,val) for k,val in self.ecp.items()]
            output += 'ecp\n'                     # Enter ecp menu
            if ecps:
                for atom,ecp in ecps:
                    output += f'"{atom}" {ecp}\n'  # Add ecps
            output += '\n'                        # Exit the ecp menu
    ...
    def write_auxbasis_submenu(self):
        ...
        if type(self.auxbasis) == str: 
            ...
            basis_definition = ''
            default_auxbasis = self.auxbasis.get('default','')
            nick_lines = []
            basis_lines = []
            for atom in self.atoms:
                auxbasis = self.auxbasis.get(atom,default_auxbasis)
                if not auxbasis:
                    raise ValueError(f'auxbasis for "{atom}" not defined')
                if self.basis != self.auxbasis: 
                    nick_lines.append(f'nick {atom} {auxbasis}\n')
                basis_lines.append(f'b "{atom}" {auxbasis}\n')
            basis_definition = ''.join(nick_lines) + ''.join(basis_lines)
        return f'jbas\n{basis_definition}*\n'
    ...
    def generate(self):
        ...
        p_define = subprocess.Popen(shlex.split('define'),
                                    ...
                                    text=True)
        out,err = p_define.communicate(define_str)
        p_define.wait()
        ...
        if cosmo_str:
            p_cosmoprep = subprocess.Popen(shlex.split('cosmoprep'),
                                           ...
                                           stderr=subprocess.DEVNULL,
                                           text=True)
            out,err = p_cosmoprep.communicate(cosmo_str)
            p_cosmoprep.wait()
